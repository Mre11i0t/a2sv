...
def mainScreen():
    ...
    showDisplay(displayMode,"                   ██║  ██║███████╗███████║ ╚████╔╝ ")
    showDisplay(displayMode,"                   ╚═╝  ╚═╝╚══════╝╚══════╝  ╚═══╝  ")
    showDisplay(displayMode,"               [Auto Scanning to SSL Vulnerability "+a2sv_version+"]")
    showDisplay(displayMode,VIOLET+"                     by HaHwul (www.hahwul.com)")
...
def runScan(s_type):
    ...
    global anonymous_result
    # SSL Check Logic --------------------------- 
    showDisplay(displayMode,"[INF] Check the SSL..")
    ...
    if "Connection refused" in result:
        showDisplay(displayMode,"[RES] This target does not support SSL..")
    # ------------------------------------------------------
    else:
        showDisplay(displayMode,"[RES] This target supports SSL..")
        if s_type == "anonymous":
            showDisplay(displayMode,"[INF] Scan Anonymous Cipher..")
            anonymous_result = m_anonymous_run(targetIP,port,displayMode)
            showDisplay(displayMode,"[RES] Anonymous Cipher :: "+anonymous_result)
        # elif s_type == "crime":
        #     showDisplay(displayMode,"[INF] Scan CRIME(SPDY)..")
        #     crime_result = m_crime_run(targetIP,port,displayMode)
        #     showDisplay(displayMode,"[RES] CRIME(SPDY) :: "+crime_result)
        elif s_type == "heart":
            showDisplay(displayMode,"[INF] Scan HeartBleed..")
            heartbleed_result = m_heartbleed_run(targetIP,port,displayMode)
            showDisplay(displayMode,"[RES] HeartBleed :: "+heartbleed_result)
        elif s_type == "ccs":
            showDisplay(displayMode,"[INF] Scan CCS Injection..")
            ccs_result = m_ccsinjection_run(targetIP,port,displayMode)
            showDisplay(displayMode,"[RES] CCS Injection :: "+ccs_result)
        elif s_type == "poodle":
            showDisplay(displayMode,"[INF] Scan SSLv3 POODLE..")
            poodle_result = m_poodle_run(targetIP,port,displayMode)
            showDisplay(displayMode,"[RES] SSLv3 POODLE :: "+poodle_result)
        elif s_type == "freak":
            showDisplay(displayMode,"[INF] Scan OpenSSL FREAK..")
            freak_result = m_freak_run(targetIP,port,displayMode)
            showDisplay(displayMode,"[RES] OpenSSL FREAK :: "+freak_result)
        elif s_type == "logjam":
            showDisplay(displayMode,"[INF] Scan OpenSSL LOGJAM..")
            logjam_result = m_logjam_run(targetIP,port,displayMode)
            showDisplay(displayMode,"[RES] OpenSSL LOGJAM :: "+logjam_result)
        elif s_type == "drown":
            showDisplay(displayMode,"[INF] Scan SSLv2 DROWN..")
            logjam_result = m_drown_run(targetIP,port,displayMode)
            showDisplay(displayMode,"[RES] SSLv2 DROWN :: "+drown_result)
        else:
            showDisplay(displayMode,"[INF] Scan Anonymous Cipher..")
            anonymous_result = m_anonymous_run(targetIP,port,displayMode)
            # showDisplay(displayMode,"[INF] Scan CRIME(SPDY)..")
            ...
            showDisplay(displayMode,"[RES] Finish scan all vulnerability..")

def outVersion():
    print "A2SV v"+a2sv_version

def updateVersion():
    print "[INF] Update A2SV"
    print "[INF] This A2SV version is .. v"+a2sv_version
    os.chdir(os.path.dirname( os.path.abspath( __file__ )))
    os.system("git reset --hard HEAD")
    os.system("git pull -v")
    vfp = open(myPath+"/version","r")  #Version File Pointer
    print "[FIN] Updated A2SV"
...
#----------- -------- -----------
    ...
    for i in data:
        for j in i:
            if j=="v_vuln":
                vulnerabilities[i[j]]={'CVE':i['v_cve'],'State':i['v_state'],'CVSS V2 Base Score':i['cvss']}
...
if args.target:
    target = args.target
    showDisplay(displayMode,"[SET] target => "+args.target)
    targetIP = socket.gethostbyname(target)
    showDisplay(displayMode,"[SET] IP Address => "+targetIP)
elif args.targetfile:
    f = open(args.targetfile,"r")
    showDisplay(displayMode,"[SET] target => "+args.targetfile)
    showDisplay(displayMode,"[SET] IP Address list")
    line = f.readline()
    while line:
        targetfileList.append(socket.gethostbyname(line.rstrip('\n')))
        showDisplay(displayMode,"       => "+str(targetfileList[-1:]))
        line = f.readline()        
    targetMode = 1
    displayMode = 1
...
if args.port:
    port = int(args.port)
    showDisplay(displayMode,"[SET] target port => "+args.port)
else:
    port = 443
    showDisplay(displayMode,"[SET] target port => 443")
if args.module:
    ...
    showDisplay(displayMode,"[SET] include => "+ModuleName+" Module")
else:
    checkVun = "all"
    showDisplay(displayMode,"[SET] include => All Module")
...
showDisplay(displayMode,"Scan Finish!")
